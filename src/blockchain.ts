...
export class Blockchain {
  ...
  minerarBloco(bloco: Bloco['payload']) {
    ...
    while (true) {
      ...
      if (hashValidado({ hash: hashPow, dificuldade: this.dificuldade, prefixo: this.prefixoPow })) {
        ...
        console.log(
          `Bloco #${bloco.sequencia} minerado: ${tempoMineracao}s. Hash ${hashReduzido} (${nonce} tentativas)`
        )
        return {
          blocoMinerado: {
            payload: { ...bloco },
  ...
  verificarBloco(bloco: Bloco): boolean {
    if (bloco.payload.hashAnterior !== this.hashUltimoBloco()) {
      console.error(
        `Bloco #${bloco.payload.sequencia} inválido: hash anterior é ${this.hashUltimoBloco()} e não ${
          bloco.payload.hashAnterior
        }`
      )
      return false
    }
    ...
    if (!hashValidado({ hash: hashTeste, dificuldade: this.dificuldade, prefixo: this.prefixoPow })) {
      console.error(
        `Bloco #${bloco.payload.sequencia} inválido: Nonce ${bloco.header.nonce} é inválido e não pode ser verificado
        }`
      )
      return false
    }
  ...
  enviarBloco(bloco: Bloco): Bloco[] {
    if (this.verificarBloco(bloco)) {
      ...
      console.log(`Bloco #${bloco.payload.sequencia} adicionado à blockchain: ${JSON.stringify(bloco, null, 2)}`)
    }
    return this.#chain
  }
}
